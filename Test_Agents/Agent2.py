    ...
    try:
        async with async_playwright() as p:
            ...
            while iteration < max_iterations:
                ...
                await asyncio.sleep(1)  # Use asyncio.sleep instead of page.wait_for_timeout
...
# ---------------- Thread-based execution for Windows compatibility ----------------
def run_agent_sync(url: str, question: str):
    """Run playwright in a separate thread with its own event loop"""
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    try:
        return loop.run_until_complete(_run_playwright_agent(url, question))
    except Exception as e:
...
async def run_agent(url: str, question: str):
    ...
    # Use thread-based execution for better Windows compatibility
    with concurrent.futures.ThreadPoolExecutor() as executor:
        result = await asyncio.get_event_loop().run_in_executor(
            executor, run_agent_sync, url, question
        )
    return result
